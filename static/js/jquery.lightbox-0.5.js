	...
	$.fn.lightBox = function(settings) {
		...
		function _start(objClicked,jQueryMatchedObj) {
			...
			if ( jQueryMatchedObj.length == 1 ) {
				settings.imageArray.push(new Array(objClicked.getAttribute('href'),objClicked.getAttribute('class').replace(/_/gi, ' ')));
			} else {
				// Add an Array (as many as we have), with href and class atributes, inside the Array that storage the images references
				for ( var i = 0; i < jQueryMatchedObj.length; i++ ) {
					settings.imageArray.push(new Array(jQueryMatchedObj[i].getAttribute('href'),jQueryMatchedObj[i].getAttribute('class').replace(/_/gi, ' ')));
				}
			}
